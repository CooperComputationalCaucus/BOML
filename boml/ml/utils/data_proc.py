...
def load_regression_data(list_IDs, dataset_path, regression_target='default',
                         dim=(2048,), target_normalization=False,
...
if __name__ == '__main__':
    ...
    params = {'val_split': .1,
              ...
              'dataset_name': 'molecule_dataset.csv',
              'shuffle': False,
              'use_generator': False,
              'data_fmt': 'npy',
              'target': 'HER',
    ...
    X, y = load_categorical_data(
        list_IDs=data['train_addrs'],
        labels=data['train_labels'],
        shuffle=params['shuffle'],
        dim=params['data_shape'],
        data_fmt=params['data_fmt']
    )
    print("\nLoad classification results")
    print('X shape', X.shape)
    print('y shape', y.shape)
    train_generator = CategoricalDataGenerator(
        list_IDs=data['train_addrs'],
        labels=data['train_labels'],
        shuffle=params['shuffle'],
        dim=params['data_shape'],
    ...
    X_r, y_r = load_regression_data(
        dataset_path=params['dataset_dir']+params['dataset_name'],
        list_IDs=data['train_addrs'],
        shuffle=params['shuffle'],
        dim=params['data_shape'],
        data_fmt=params['data_fmt'],
        regression_target=params['target']
    )
